...
Voxelgrid::FloatVector Voxelgrid::ToFloatVector() const
{
  FloatVector result(m_width * m_height * m_depth);
  for (uint64 z = 0; z < m_depth; z++)
    for (uint64 y = 0; y < m_height; y++)
      for (uint64 x = 0; x < m_width; x++)
      {
        const uint64 i = x + y * m_width + z * m_width * m_height;
        result[i] = at(x, y, z);
      }
...
Voxelgrid::Ptr Voxelgrid::FromFloatVector(const FloatVector & data, const uint64 width, const uint64 height, const uint64 depth)
{
  Voxelgrid::Ptr result(new Voxelgrid(width, height, depth));
  if (width * height * depth != data.size())
  ...
  for (uint64 z = 0; z < depth; z++)
    for (uint64 y = 0; y < height; y++)
      ...
      {
        const uint64 i = x + y * width + z * width * height;
        result->at(x, y, z) = data[i];
      }
...
{
  if (iterations == Eigen::Vector3i::Zero())
    return Voxelgrid::Ptr(new Voxelgrid(*this));

  const Eigen::Vector3i scale(1ul << (iterations.x()),
                              1ul << (iterations.y()),
                              1ul << (iterations.z()));
...
template <class T>
Voxelgrid::Ptr Voxelgrid::FromMultiArray(const T & arr)
{
...
template <class T>
T Voxelgrid::ToMultiArray() const
{
  T result;
...
{
  return ToMultiArray<std_msgs::Float32MultiArray>();
}

Voxelgrid::Ptr Voxelgrid::FromFloat32MultiArray(const std_msgs::Float32MultiArray & arr)
{
  return FromMultiArray(arr);
}

std_msgs::Int8MultiArray Voxelgrid::ToInt8MultiArray() const
{
  return ToMultiArray<std_msgs::Int8MultiArray>();
}

Voxelgrid::Ptr Voxelgrid::FromInt8MultiArray(const std_msgs::Int8MultiArray & arr)
{
  return FromMultiArray(arr);
}

std_msgs::UInt8MultiArray Voxelgrid::ToUInt8MultiArray() const
{
  return ToMultiArray<std_msgs::UInt8MultiArray>();
}

Voxelgrid::Ptr Voxelgrid::FromUInt8MultiArray(const std_msgs::UInt8MultiArray & arr)
{
  return FromMultiArray(arr);
}
